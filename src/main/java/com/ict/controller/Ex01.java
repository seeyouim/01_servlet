...
public class Ex01 extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 맴버필드 초기화
		// init()가 끝나면 자동으로 service()를 호출 한다.
		System.out.println("init");
		
	}

	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// request(요청) = 요청정보를 가지고 있음, response(응답) = 응답정보를 가지고 있다.
		// 클라이언트가 정보를 요청할때 요청 방식에 따라 알맞는 메서드를 호출하는 역할.
		...
		//					   주소창에 정보가 보이지 않는다.
		//					   속도는 get방식보다 느리나, 많은 양을 전달 할 수 있다.
		System.out.println("service");
		
		// doGet(), doPost()에 요청방식에 맞게 호출한다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 해당 프로젝트가 톰켓에서 삭제 되기  직전 실행되는 메서드
	public void destroy() {
